// let start: number = 0;          // 记录第一次调用step函数的时间点，用于计算与第一次调用step函数的时间差，以毫秒为单位
// let lastTime: number = 0;       // 记录上一次调用step函数的时间点，用户计算两帧之间的时间差，以毫秒为单位
// let count: number = 0;          // 记录step函数被调用的次数
